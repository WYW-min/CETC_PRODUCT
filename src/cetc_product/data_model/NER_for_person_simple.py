...
class EducationAndCareer(BaseModel):
    ...
    education_history: List[EducationRecord] = Field(
        default_factory=list, description="学历、毕业院校、专业、学位"
    )
...
class HealthAndLifestyle(BaseModel):
    ...
    sleep_duration: Measurement | None = Field(
        None, description="睡眠时长 (单位: 小时)"
    )
...
class FinancialProfile(BaseModel):
    ...
    vehicle_ownership: List[str] = Field(
        default_factory=list, description="车辆拥有情况"
    )
    ...
    investment_preferences: List[str] = Field(
        default_factory=list, description="投资偏好"
    )
...
class PersonalInterests(BaseModel):
    ...
    shopping_preferences: List[str] = Field(
        default_factory=list, description="购物偏好"
    )
    media_consumption_habits: List[str] = Field(
        default_factory=list, description="媒体消费习惯"
    )
    ...
    transportation_preferences: List[str] = Field(
        default_factory=list, description="交通方式偏好"
    )
...
class DigitalFootprint(BaseModel):
    ...
    social_media_accounts: List[SocialMediaAccount] = Field(
        default_factory=list, description="社交媒体账号"
    )
    ...
    browsing_history: List[str] = Field(
        default_factory=list, description="网络浏览历史"
    )
    ...
    online_purchase_history: List[str] = Field(
        default_factory=list, description="在线购买记录"
    )
...
class GeographicInformation(BaseModel):
    ...
    household_registration_location: Location | None = Field(
        None, description="户籍所在地"
    )
...
# --- 3. 最终的顶层输出模型 ---
class PersonInfos(BaseModel):
    """从页面中尽力抽取的、详尽的个人信息范式模型。"""

    consolidated_view: List[PersonAttributes] | None = Field(
        None,
        description="所有来源抽取结果的合并与去重视图，若无有效信息或并非一个真实的人物则为null。",
    )
    why: str | None = Field(None, description="简要说明抽取信息")
