...
import asyncio
from typing import Callable, Dict, List, Protocol, Any, TypeVar, Generic, runtime_checkable
...
class BaseLangChainTask(ABC):
    ...
    def __init__(self, chain: object, task_name: TaskTypeEnum):
        ...
        assert isinstance(task_name, TaskTypeEnum), f"task_name必须为类型TaskTypeEnum，当前输入类型：{type(task_name)}"
        self._chain = chain
    ...
    def run(self, input_data: Dict|List[Dict], run_mode:ChainRunModeEnum = ChainRunModeEnum.INVOKE) -> Any | List[Any]:
        ...
        return [self.get_output(in_data, out_data) for in_data, out_data in zip(input_data, chain_outputs)]
