...
class MyChain:
    ...
    def __init__(self, prompt_path:Path, data_model:BaseModel,llm_name = "qwen"):
        ...
        self.chain = self.prompt_temp.partial(schema_define=self.syn_parser.get_format_instructions()) | self.main_llm  | self.syn_parser
    ...
    async def abatch(self, input_datas):
        ...
        final_data = [self.get_final_data(run_chain_result) for run_chain_result in ga_coroutine_results]
        return final_data
