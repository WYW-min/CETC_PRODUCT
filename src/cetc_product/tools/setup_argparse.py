...
from cetc_product.tools.tiny_tool import get_globpath, safe_get
...
class ConfigLoader:
    ...
    def _enhance_config(
        self, task_key: TASK_KEYS, task_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        enhanced_config = task_config.copy()

        if "outpath" in enhanced_config:
            enhanced_config["outpath"] = str(
                Path(enhanced_config["outpath"]) / task_key[0] / task_key[1]
            )

        if "checkpoint_path" in enhanced_config:
            enhanced_config["checkpoint_path"] = str(
                Path(enhanced_config["checkpoint_path"]) / task_key[0] / task_key[1]
            )
        ...
        for p in glob_paths:
            parsed_paths += [str(_p) for _p in get_globpath(Path(p))]

        enhanced_config["writed_data_paths"] = sorted(parsed_paths)
...
示例:
  ...
  python -m cetc_product.core.pipeline --task task1 --source zhwiki --log-dir ./logs
    ...
    # 必需参数
    parser.add_argument(
        "--task", "-t", type=str, required=True, help="任务类型 (如: task1, task2)"
    )

    parser.add_argument(
        "--source", "-s", type=str, required=True, help="数据源 (如: zhwiki, enwiki)"
    )
    ...
    parser.add_argument("--test", action="store_true", help="测试模式,只处理一批数据")
    ...
    parser.add_argument("--inpath", type=str, help="输入路径 (覆盖配置文件)")
