...
from cetc_product.tools.time_provider import (
    TimestampProvider,
...
def count_json_lines_parallel(
    inpaths: List[Path], filter_func: Callable[[Any], bool], n_workers: int = None
) -> int:
    """
    并行统计多个文件的 JSON 行数
...
def read_data(
    inpaths: List[Path], filter_func: Callable[[Any], bool], buffer_size: int = 2000
) -> GE_DATA:
            ...
            with xopen.xopen(current_path, "r") as fin:
                ...
                for line_no, line in enumerate(fin, 1):
                    ...
                    try:
                        ...
                        if filter_func(data):
                            buffer.append((current_path, data))
...
def load_prompt(
    prompt_path: Path | str, schema_define_cls: BaseModel | None = None
) -> PromptTemplate:
    ...
    if (
        "schema_definition" in prompt_temp.input_variables
        and schema_define_cls is not None
    ):
        prompt_temp = prompt_temp.partial(
            schema_definition=MyJSONParser(schema_define_cls).get_format_instructions()
        )
...
def enhance_inpath(inpath: Path) -> Path | List[Path]:
    inpaths = sorted(list(get_globpath(inpath)))
    return inpaths


def enhance_outpath(
    outpath: Path, timestamp_provider: TimestampProvider | None = None
) -> Path:
    provider = timestamp_provider or default_timestamp_provider
    now_str = provider.format("%Y%m%d_%H%M%S")
    if outpath.suffix == "":
