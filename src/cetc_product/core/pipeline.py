...
class Pipeline:
    ...
    def __init__(self, log_dir: Path | str = "./logs"):
        ...
        self.cur_task = None
        self.task = None
        self.cur_params = {"io_params": None, "chain_params": None}
        logger.success("Pipeline 初始化完成")

    def bind(
        self, task_type: TaskTypeEnum, task_params: Dict[str, str]
    ...
    def _init_task1(self, task_params: Dict[str, Any]):
        ...
        task_params = self._task_params_parse(
            task_params, require_params, optional_params
        )
        # 记录参数
    ...
    def _init_task2(self, task_params: Dict[str, Any]):
        # 扫描参数
        require_params = {"inpath", "outpath", "prompt_path"}
        optional_params = {"read_batch_size": 2, "llm_name": "doubao_flash"}
        task_params = self._task_params_parse(
            task_params, require_params, optional_params
        )
        self._init_checkpoint(task_params)
                ...
                for inpath_list, indata_list in read_data_batched(
                    ...
                    if self.checkpoint:
                        filtered_data, skipped = self.checkpoint.filter(
                            indata_list, get_id=Funcs.wiki_dict_getid
                        )
                    ...
                    task_response, read_buffer = self._task_submit(
                        read_buffer,
                        read_batch_size if not test else len(read_buffer),
                        fout,
                    )
                    ...
                    if self.checkpoint and task_response:
                        ...
                        logger.info(
                            f"本批成功: {marked} / {checkpoint_log['total_processed']} 条, 跳过: {failed} / {checkpoint_log['total_skipped']} 条"
                        )
                    if test:
